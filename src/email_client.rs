...
mod tests {
    ...
    impl wiremock::Match for SendEmailBodyMatcher {
        fn matches(&self, request: &Request) -> bool {
            ...
            if let Ok(body) = result {
                dbg!(&body);
                //Check that all themandatory fields are populated
    ...
    fn email_client(base_url: String) -> EmailClient {
        EmailClient::new(base_url, email(), SecretString::new(Faker.fake::<String>().into_boxed_str()), std::time::Duration::from_millis(1000))
    }

    #[tokio::test]
    pub async fn send_email_sends_expected_request() {
        // Arrange
